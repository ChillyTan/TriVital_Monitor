...
* 返 回 值：void
* 创建日期：2018年01月01日
* 注    意：为避免寄生干扰和额外功耗，GPIO模式建议设置为模拟输入 PA4
*********************************************************************************************************/
static  void ConfigDAC1(void)
{
  ...
  DAC_SetChannel1Data(DAC_Align_12b_R, 450);        //设置为12位右对齐数据格式 250
...
{  
  ...
  //配置DMA2_Channel3
  DMA_DeInit(DMA2_Channel3);  //将DMA2_CH3寄存器设置为默认值
...
void InitDAC(void)
{              
//  s_strDAC1WaveBuf.waveBufAddr  = (u32)GetSPO2Wave100PointAddr(); //波形地址
//  s_strDAC1WaveBuf.waveBufSize  = 1;                            //波形点数  
