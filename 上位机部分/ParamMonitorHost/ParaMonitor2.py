...
class ParamMonitor(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def slot_serial(self, portNum, baudRate, dataBits, stopBits, parity):
        ...
        if self.ser.isOpen():
            ...
            self.statusBar().showMessage(self.statusStr)
            self.serialPortTimer.start(20)  # 开启处理串口数据的定时器（适当增加间隔）
            self.procDataTimer.start(50)  # 开启处理已解包数据的定时器（适当增加间隔）
    ...
    def drawRespWave(self):
        ...
        # 临时创建 QPainter 对象
        painter = QPainter(self.pixmapResp)
    ...
    def drawSPO2Wave(self):
        ...
        # 临时创建 QPainter 对象
        painter = QPainter(self.pixmapSPO2)
        painter.setBrush(Qt.white)
        painter.setPen(QPen(Qt.white, 1, Qt.SolidLine))
    ...
    def drawECG1Wave(self):
        ...
        # 临时创建 QPainter 对象
        painter = QPainter(self.pixmapECG1)
        painter.setBrush(Qt.white)
        painter.setPen(QPen(Qt.white, 1, Qt.SolidLine))
    ...
    #
    #     # 临时创建 QPainter 对象
    #     painter = QPainter(self.pixmapECG2)
    #     painter.setBrush(Qt.white)
    #     painter.setPen(QPen(Qt.white, 1, Qt.SolidLine))
    #
    ...
    #
    #     # 设置画笔
    #     painter.setPen(QPen(Qt.black, 2, Qt.SolidLin
    ...
    def loadFile(self):
        ...
        if self.mTimerStartFlag:
            self.proLoadDataTimer.start(20)
            self.procDataTimer.start(50)
